...
副作用とは、ある機能が 1 つの処理である値の状態を変化させる事で、それ以降の処理で得られる結果に影響を与えることを言います。
...
引数を 2 倍して返す単純な関数ですが、ここに渡す引数を「2」として、 for 文で 10 回ループさせたとします。
...
あるグローバルな値があるとして、それに任意の引数を加算して返す関数ですが、渡す引数を「1」として for 文で 10 回ループさせたとします。
